...
namespace SocialCareCaseViewerApi.Tests.V1.Controllers
{
    ...
    public class SocialCareCaseViewerApiControllerTests
    {
        ...
        {
            var allocationList = _fixture.Create<AllocationList>();
            var listAllocationsRequest = new ListAllocationsRequest();

            _mockGetAllocationUseCase.Setup(x => x.Execute(listAllocationsRequest)).Returns(allocationList);
            var response = _classUnderTest.GetAllocatedWorker(listAllocationsRequest) as OkObjectResult;

            response.Should().NotBeNull();
            response.StatusCode.Should().Be(200);
            response.Value.Should().BeEquivalentTo(allocationList);
        }
