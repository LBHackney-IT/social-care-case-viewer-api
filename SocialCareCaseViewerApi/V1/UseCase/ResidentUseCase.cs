...
namespace SocialCareCaseViewerApi.V1.UseCase
{
    public class ResidentUseCase : IResidentUseCase
    {
        ...
        {
            _databaseGateway.AllocateResidentToTheTeam(request);
        }
        ...
        {
            List<ResidentInformation> residents = new List<ResidentInformation>();
            var matchingPersons = _databaseGateway.GetPersonsByTeamId(rqp.TeamId);
            if (matchingPersons != null)
            {
                foreach (var person in matchingPersons)
                {
        ...
        public ResidentInformationList GetWaitingListByQuery(ResidentQueryParam rqp, int cursor, int limit)
        {
            long? mosaicId = null;
            int totalCount = 0;
